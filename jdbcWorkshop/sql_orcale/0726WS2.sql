...
-- 이름, 급여, 경조비(소수점 이하 절삭)를 출력하라.
 ...
 SELECT ename, sal, ROUND(sal*0.05) 경조비 FROM emp WHERE sal >= 2000;
...
SELECT ename, ADD_MONTHS(hiredate,6) "지난날짜", sal FROM emp;

-- 3) 입사일로부터 오늘까지의 일수를 구하여 이름, 입사일, 근무일수를 출력하라.

SELECT ename, hiredate, TRUNC(sysdate-TO_DATE(hiredate, 'RR-MM-DD')) 근무일수 FROM emp;

-- 4) 직원의 이름, 근속년수를 구하여 출력하라.

SELECT ename, hiredate, TRUNC(TRUNC(sysdate-TO_DATE(hiredate))/365) 근속년수 FROM emp;
...
SELECT ename, TO_CHAR(to_date(hiredate),'DAY') 입사요일 FROM emp;
